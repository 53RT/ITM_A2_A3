...
public class VideoThumbnailGenerator {
	...
	protected void getImagesFromVideo(File input, int timeSpan){
		...
		if (container.open(filepath, IContainer.Type.READ, null) < 0)
		      throw new IllegalArgumentException("Datei konnte nicht ge�ffnet werden " + filepath);
	    ...
	    //Diesen Abschnitt nur durchgehen wenn TimeSpan es zul�sst.
    	...
    	if(timeSpan > 0){
    	 ...
    	 for(int i = 0; i < (numPictures-1); i++){
    		 if (startWert == allPictures.get(i).getPts()/1000000){
    			 ...
    			 //startWert auf n�chstgr��eren erh�hen;
    	...
    	if(timeSpan == 0){
    		...
    		//i Muss aus Perfomancegr�nden in gr��eren Zeitschritten erh�ht werden
    		...
    		for(int j = i; j < allPictures.size(); j = j + 4){
    			...
    			if(tool.match() == false ){
    				...
    				//Bild J hinzuf�gen und I auf J stellen
